compare2otherMethods(testData,'Net', net, 'Numeric', true);
...
%   'Numeric'   : true (default) if to calculate numeric outputs afet Quit
%
% Notes:
...
addParameter(p,'Numeric',true,            @(s)islogical(s));
parse(p, varargin{:});
args = p.Results;
...
for i = 1:N
    ...
    try
        ...
        sensorsK  = S{3};
        [H,W]     = size(Xtrue);
        % Xmin = log10(ramp(min(Xtrue(:))));
        % Xmax = log10(scallingCorrection(max(Xtrue(:)))) + 1e-16;
        Xmin = 0;
        Xmax = max(scallingCorrection(Xtrue(:))) + 1e-12;
        ...
        if interactive
            fig = figure('Color','w','Position',[80 80 800 800]);
            switch lower(string(args.Layout))
                case "4x2", rows = 4; cols = 2;
                otherwise,  rows = 2; cols = 4; % default
            end
            tiledlayout(fig, rows, cols, 'TileSpacing','compact','Padding','tight');
    
            show_panel(scallingCorrection(Y_lin), Xs, Ys, Xmin, Xmax, sprintf('Linear   r=%.2f', CORR(i,1)));
            show_panel(scallingCorrection(Y_nat), Xs, Ys, Xmin, Xmax, sprintf('Natural  r=%.2f', CORR(i,2)));
            show_panel(scallingCorrection(Y_nn),  Xs, Ys, Xmin, Xmax, sprintf('Nearest  r=%.2f', CORR(i,3)));
            show_panel(scallingCorrection(Y_idw), Xs, Ys, Xmin, Xmax, sprintf('IDW      r=%.2f', CORR(i,4)));
    
            if M >= 5
                if isnan(CORR(i,5)), ttl5 = 'Universal Kriging (N/A)'; else, ttl5 = sprintf('Universal Kriging  r=%.2f', CORR(i,5)); end
                show_panel(scallingCorrection(Y_uk),  Xs, Ys, Xmin, Xmax, ttl5);
            end
            if M >= 6
                if isnan(CORR(i,6)), ttl6 = 'Ordinary Kriging (N/A)'; else, ttl6 = sprintf('Ordinary Kriging   r=%.2f', CORR(i,6)); end
                show_panel(scallingCorrection(Y_ok),  Xs, Ys, Xmin, Xmax, ttl6);
            end
            if hasNet
                ttl7 = sprintf('RSIM     r=%.2f', CORR(i,7));
                show_panel(scallingCorrection(Y_rsim), Xs, Ys, Xmin, Xmax, ttl7);
            end
    
            nexttile; imagesc(scallingCorrection(Xtrue), [Xmin Xmax]); title('Ground truth'); axis image off; 
            cb = colorbar;
            cb.Layout.Tile = 'east';
